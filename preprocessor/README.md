...
Status: internal `PRE-002` through `PRE-004`, `SASS-010` through `SASS-012`, `LESS-010` through `LESS-012`, and `STYLUS-010`; `STYLUS-011` is active. SCSS, indented Sass, and Less have passed their internal exact-provider, confined-import, pinned-corpus, negative, limit, determinism, and generated-CSS gates. The exact Stylus renderer now has confined ordinary imports, safe renderer options, dependency ownership, imported diagnostics/maps, and resolver-backed built-in file helpers; its independent corpus and final graduation gates remain incomplete. All four syntaxes are still publicly unavailable until their remaining product, packaging, platform, and per-row graduation gates pass.
...
| `stylus` | `stylus` | `stylus` `0.64.0` | `STYLUS-011`; confined integration active, corpus pending |
...
This remains internal pre-admission work, not full Stylus graduation. `STYLUS-011` must finish its complete negative/limit and repository-wide validation before it can be marked verified. `STYLUS-012` must then pin and pass an authoritative positive/negative corpus with direct canonical differentials and generated-CSS validation. `.styl` remains rejected by the public CLI until those packages plus `PRE-005`, `PRE-006`, and per-row `PRE-008` pass.
