...
Status: internal `PRE-002` through `PRE-004`, `SASS-010` through `SASS-012`, `LESS-010` through `LESS-012`, and `STYLUS-010` through `STYLUS-012`. SCSS, indented Sass, Less, and Stylus have passed their internal exact-provider, confined-import, pinned-corpus, negative, limit, determinism, and generated-CSS gates. All four syntaxes are still publicly unavailable until their remaining CLI, batch/watch, packaging, platform, consumer, and per-row documentation graduation gates pass.
...
| `stylus` | `stylus` | `stylus` `0.64.0` | `STYLUS-012`; canonical conformance internal only |
...
Twenty-nine official cases remain explicit non-passing evidence: five require JavaScript plugins or injected resolver callbacks, five require the undocumented renderer `prefix` hook, and nineteen emit legacy or intentionally invalid CSS rejected by the independent parser. They are retained in the checksum-owned source snapshot and named exclusion ledger rather than counted as passes. This closes internal Stylus canonical-language conformance, not public admission, arbitrary hook support, or ecosystem-plugin parity. `.styl` remains rejected by the public CLI until `PRE-005`, `PRE-006`, and per-row `PRE-008` product, package, platform, batch/watch, consumer, and documentation gates pass.
...
The pre-admission Dart Sass, Less, and Stylus adapters consume this loader for every project dependency byte. These boundaries remain the file and result authority across later corpus and product graduation. This work does not make any preprocessor syntax available publicly.
