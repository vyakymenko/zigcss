...
test('canonical versions and release tags fail closed', () => {
  assert.deepEqual(parseReleaseVersion('0.4.0-rc.1'), {
    value: '0.4.0-rc.1',
    base: '0.4.0',
    prerelease: 'rc.1',
    build: null,
  ...
  assert.equal(validateReleaseTag('0.4.0-rc.1', 'v0.4.0-rc.1'), true)
  assert.throws(() => validateReleaseTag('0.4.0-rc.1', 'v0.4.0'), /release tag must be v0\.4\.0-rc\.1/)

  const plan = cloneSources()
  replace(plan, 'DEVELOPMENT_PLAN.md', 'Target: `0.4.0-rc.1`', 'Target: `0.4.0-rc.2`')
  assert.throws(() => validateReleaseSources(plan), /VERSION must be 0\.4\.0-rc\.2, received "0\.4\.0-rc\.1"/)
})

test('manifest, lockfile, Zig, CLI, and Marketplace mapping drift fails closed', () => {
  const rootLock = cloneSources()
  replace(rootLock, 'package-lock.json', '"version": "0.4.0-rc.1"', '"version": "9.9.9"')
  assert.throws(() => validateReleaseSources(rootLock), /root npm lock version/)

  const docsLock = cloneSources()
  replace(docsLock, 'docs/package-lock.json', '"version": "0.4.0-rc.1"', '"version": "9.9.9"')
  assert.throws(() => validateReleaseSources(docsLock), /documentation linked ZigCSS version/)

  const vscode = cloneSources()
  replace(vscode, 'vscode-extension/package.json', '"version": "0.4.0"', '"version": "0.4.0-rc.1"')
  assert.throws(() => validateReleaseSources(vscode), /VS Code Marketplace package version/)

  const zig = cloneSources()
  replace(zig, 'build.zig.zon', '.version = "0.4.0-rc.1"', '.version = "9.9.9"')
  assert.throws(() => validateReleaseSources(zig), /Zig package version/)

  const cli = cloneSources()
  replace(cli, 'src/main.zig', 'const version = "0.4.0-rc.1";', 'const version = "9.9.9";')
...
test('CI ordering, release-tag preflight, and VS Code prerelease packaging fail closed', () => {
  ...
  inventory.set('unowned-version.txt', '0.4.0-rc.1\n')
