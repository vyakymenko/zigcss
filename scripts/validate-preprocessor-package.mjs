...
function readJson(filename, label) {
  try {
    return JSON.parse(readStableUtf8File(filename, {
      label,
...
function readRepositoryText(root, relativePath, label = relativePath) {
  const file = regularFile(root, relativePath, label)
  return readStableUtf8File(file.filename, file.options)
}
...
export function discoverRuntimeSourceClosure(root = repositoryRoot) {
  ...
  while (pending.length !== 0) {
    ...
    const file = regularFile(root, relativePath, `runtime source ${relativePath}`)
    const filename = file.filename
    const source = readStableUtf8File(filename, file.options)
    ...
    for (const reference of sourceReferences(source)) {
      if (reference.startsWith('.')) {
        ...
        if (target === 'package.json') {
          const metadata = regularFile(root, target, 'runtime package metadata')
          readStableUtf8File(metadata.filename, metadata.options)
          continue
        }
        pending.push(target)
...
function validateGenerated(root, relativePath, expected) {
  const file = regularFile(root, relativePath, relativePath)
  const actual = readStableUtf8File(file.filename, file.options)
  if (actual !== expected) fail(`${relativePath} is stale; run npm run generate:preprocessor-package`)
}

export function writeGeneratedFileAtomically(root, relativePath, contents) {
  if (!generatedPackageFiles.includes(relativePath)) fail('generated output path is not allowlisted')
  const filename = path.resolve(root, relativePath)
  const relative = path.relative(path.resolve(root), filename)
  if (relative === '..' || relative.startsWith(`..${path.sep}`) || path.isAbsolute(relative)) {
    fail('generated output path escapes the repository')
  }
  const label = relativePath
  const bytes = Buffer.from(contents)
  if (bytes.length === 0 || bytes.length > maximumGeneratedBytes) {
    fail(`${label} generated bytes are outside the allowed range`)
  }
  const existing = readStableUtf8File(filename, {
    allowMissing: true,
    label,
    maximumBytes: maximumGeneratedBytes,
    reject: fail,
  ...
  try {
    ...
    while (offset < bytes.length) {
      ...
      if (count === 0) fail(`${label} temporary write ended early`)
      offset += count
    }
    fs.fsyncSync(descriptor)
    const written = fs.fstatSync(descriptor, { bigint: true })
    if (!written.isFile() || written.size !== BigInt(bytes.length)) fail(`${label} temporary write was not exact`)
...
function validateBuildPackageNodeJobs(build) {
  const exactNodeLine = "          node-version: '24.20.0'"
  for (const jobName of ['build', 'native-provenance-evidence', 'native-package-evidence']) {
    ...
    if (!same(nodeVersionLines, [exactNodeLine])) {
      fail(`build workflow job ${jobName} must use exact Node 24.20.0`)
    }
  }
  const testNodeVersionLines = workflowJob(build, 'test')
    .split('\n')
    .filter(line => /^\s+node-version:/.test(line))
  if (!same(testNodeVersionLines, ["          node-version: '24.20.0'"])) {
    fail('build workflow job test must use exact Node 24.20.0 LTS for the pinned Next.js Webpack, Astro, and Nuxt host engines')
  }
}

function validateReleasePackageNodeJobs(release) {
  const exactNodeLine = "          node-version: '24.20.0'"
  for (const jobName of ['npm-preflight', 'release', 'create-release', 'publish-npm', 'anonymous-public-delivery']) {
    const nodeVersionLines = workflowJob(release, jobName)
      .split('\n')
      .filter(line => /^\s+node-version:/.test(line))
    if (!same(nodeVersionLines, [exactNodeLine])) {
      fail(`all release npm surfaces must use exact Node 24.20.0 LTS; job ${jobName} changed`)
    }
  }
}
...
export function validatePreprocessorPackagingWorkflows(build, release, docs) {
  ...
  if (
    literalCount(release, '    environment:\n      name: immutable-release') !== 1
    || release.includes('IMMUTABLE_RELEASES_READ_TOKEN')
  ) {
    fail('release publication must use one immutable-release approval environment without a stored administration credential')
  }
  if (literalCount(docs, "node-version: '24.20.0'") !== 1) {
    fail('documentation package consumer must use exact Node 24.20.0 LTS')
  }
  if (
    literalCount(release, 'npm publish "$NPM_PACKAGE_ARCHIVE" --tag "$RELEASE_CHANNEL" --registry=https://registry.npmjs.org/ --provenance') !== 1
...
export function validatePreprocessorPackage(root = repositoryRoot, { pack = true } = {}) {
  ...
  validatePackageManagerMatrixSource(
    readRepositoryText(root, 'scripts/verify-package-managers.test.mjs'),
  )
  ...
  validatePreprocessorPackagingWorkflows(
    readRepositoryText(root, '.github/workflows/build.yml'),
    readRepositoryText(root, '.github/workflows/release.yml'),
    readRepositoryText(root, '.github/workflows/docs.yml'),
  )
...
export function writePreprocessorPackageMetadata(root = repositoryRoot) {
  ...
  writeGeneratedFileAtomically(root, generatedPackageFiles[0], renderPreprocessorSbom(manifest, lock, closure))
  writeGeneratedFileAtomically(root, generatedPackageFiles[1], renderThirdPartyNotices(lock, closure))
