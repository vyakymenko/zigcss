...
test('release metadata is deterministic, bounded SPDX 2.3 with exact SHA-256 subjects', () => {
  ...
  try {
    ...
    assert.deepEqual(first.assets, {
      archive: 'zigcss-v0.5.0-rc.1-x86_64-linux.tar.gz',
      sbom: 'zigcss-v0.5.0-rc.1-x86_64-linux.spdx.json',
      checksums: 'zigcss-v0.5.0-rc.1-x86_64-linux.sha256',
      provenanceBundle: 'zigcss-v0.5.0-rc.1-x86_64-linux.provenance.sigstore.jsonl',
      sbomBundle: 'zigcss-v0.5.0-rc.1-x86_64-linux.sbom.sigstore.jsonl',
    })
...
test('release workflow evidence fails closed when authority or artifact steps drift', () => {
  ...
  assert.throws(
    () => validateReleaseWorkflowSource(workflow.replace(
      '- name: Publish to npm',
      '- name: Rewrite package version\n        run: npm version 0.5.0-rc.1 --no-git-tag-version\n\n      - name: Publish to npm',
    )),
