...
- Milestone 10 and NATIVE-009 are verified. The REL-010 stable-promotion sequence is active. Select the earliest dependency-eligible REL-010 slice not marked VERIFIED in DEVELOPMENT_STATUS.md and follow DEVELOPMENT_PLAN.md section 18 exactly: machine contract and release policy; stable source identity plus truthful docs, site, and SEO; complete local gates; exact origin/main integration and hosted gates; one immutable tag; workflow publication; then post-publication verification. A pending external BENCH-007 runner must not block REL-010 because comparative and superlative performance claims remain withdrawn until a controlled non-emulated Linux x64 archive exists.
...
- Do not push ordinary branch checkpoints from this model pass. The outer Bash supervisor alone may non-force push every independently verified clean checkpoint to the approved origin recovery branch and integrate origin/main in bounded batches or at completion/release validation. Before the exact `REL-010` release gate, do not publish, deploy, create a PR/tag/release, send messages, spend money, use secrets, or modify any other external system.
- The owner separately authorized on 2026-08-18 the exact stable `v0.6.0` promotion. Only after every pre-tag `REL-010` local, hosted, documentation, release, artifact, provenance, and consumer gate passes; `release/stable-promotion.json` reports the exact unused candidate with `stableReleaseReady: true`; and that exact candidate commit is already integrated to `origin/main`, the release phase may create and push `v0.6.0` exactly once. The same or later pass may monitor the fail-closed `.github/workflows/release.yml` and record the resulting non-prerelease GitHub Release, npm `latest`, and the existing GitHub Pages deployment. Never move or recreate `v0.6.0-rc.2`, move or reuse any tag, publish another npm channel, bypass a workflow or failed gate, publish Homebrew/editor-extension/container/service channels, or restore comparative performance claims without controlled BENCH-007 evidence. A failed immutable candidate must be stopped and ledgered rather than rewriting public history.
