...
- Use only gpt-5.6-sol with ultra reasoning, as selected by the invoking runner. Never delegate, spawn subagents, create child tasks, or fall back to another model.
...
- Milestone 9 canonical frontend work is active. Select the earliest dependency-eligible package not marked VERIFIED in DEVELOPMENT_STATUS.md, in this order: SCSS/Sass through SASS-012, Less through LESS-012, Stylus through STYLUS-012, then PRE-005/PRE-006/PRE-008/PRE-009. After PRE-008 is verified, PRE-009 is next. A pending external BENCH-007 runner must not block eligible preprocessor correctness work.
