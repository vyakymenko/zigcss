...
- Milestone 10, NATIVE-009, and the REL-010 stable v0.6.0 publication terminal are verified and closed. The one-time stable publication authority is consumed. The current autonomous program is complete under ADR-016; never reopen compiler, documentation, site, package, tag, GitHub Release, npm publication, or performance-claim work merely to remain busy.
- BENCH-007 is deferred external, not an active blocker. Preserve its withdrawn report and strict report/archive schema v2 bare-metal attestation. Do not provision infrastructure or spend project funds. A future controlled archive requires an explicit new opt-in under DEVELOPMENT_PLAN.md section 19; without that amended state, emit COMPLETE after read-only orientation and do not invent a progress package or commit.
- If the roadmap is explicitly reopened later, execute at most one smallest dependency-ordered work package in a pass: reproduce or measure, add/strengthen tests, implement the smallest correct change, run proportionate gates, inspect the bounded sibling surface, update the ledger, and commit intentional green checkpoints.
- Every PROGRESS pass must name one stable release-gap family tied to an explicit milestone exit criterion and reduce or close it. A sibling search is inspection, not an automatic work queue. Repeated numeric or ordinal N+1 expansion is forbidden unless the plan or ledger declares a spec-, provider-, or resource-derived predeclared finite terminal bound. Prefer representative lower-bound, terminal-bound, and over-limit tests or generated parameterization over cloned fixtures for every intermediate integer.
...
- Do not push ordinary branch checkpoints from this model pass. The outer Bash supervisor alone may non-force push every independently verified clean checkpoint to the approved origin recovery branch and integrate origin/main in bounded batches or at completion validation. Do not publish, deploy, create a PR/tag/release, send messages, spend money, use secrets, register infrastructure, or modify any other external system.
- Stable publication authority was successfully consumed and closed on 2026-08-18. Never move, delete, recreate, or republish `v0.6.0-rc.2`, `v0.6.0`, npm `0.6.0-rc.2`, or npm `0.6.0`; never reuse any tag, publish another npm channel, bypass a workflow or failed gate, or publish Homebrew/editor-extension/container/service channels. Changed package bytes or any future release require a new owner-approved SemVer identity outside this terminal. Comparative performance claims remain withdrawn until controlled BENCH-007 evidence passes every section 19 gate.
