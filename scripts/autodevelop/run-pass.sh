...
- Milestone 10, NATIVE-009, and the REL-010 stable v0.6.0 publication terminal are verified and closed. The one-time stable publication authority is consumed. Only BENCH-007 remains; follow DEVELOPMENT_PLAN.md section 19 exactly and never reopen compiler, documentation, site, package, tag, GitHub Release, or npm publication work merely because controlled benchmark evidence is pending.
- Begin BENCH-007 with read-only orientation for a newly retained archive from the schedule-only workflow on the exact current origin/main commit and dedicated non-emulated Linux x64 runner. A self-hosted label is not hardware proof: accept only report/archive schema v2 with the complete machine-generated `verified-bare-metal` attestation required by section 19. If no qualifying archive exists after safe in-scope checks, emit BLOCKED with stable code controlled-benchmark-archive, do not invent a progress package or commit, and preserve the withdrawn report. If an archive exists, execute only the smallest dependency-ordered section 19 validation/publication slice.
