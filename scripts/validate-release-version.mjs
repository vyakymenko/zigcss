...
export function validateReleaseSources(sources) {
  ...
  expectContains(sources.get('NPM_PUBLISH.md'), `Stable \`zigcss@${version}\` is published on npm \`latest\``, 'npm publishing guide stable identity')
  ...
  expectContains(sources.get('docs/src/content/docs/guide/format-compatibility.md'), `Stable ${version} contains`, 'format guide stable identity')
  expectContains(sources.get('docs/src/content/docs/guide/recovery-cli.md'), `Stable ${version} contains`, 'CLI guide stable identity')
  expectContains(sources.get('docs/src/app/components/GettingStarted.tsx'), `ZigCSS ${version}`, 'getting-started stable identity')
  expectContains(sources.get('docs/src/app/components/Home.tsx'), `Stable ${version} is published from one immutable release workflow`, 'homepage stable identity')
  ...
  expectContains(changelog, `\`zigcss@${version}\` on npm \`latest\` with provenance`, 'stable changelog channel')
