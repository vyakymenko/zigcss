...
test('an absent canonical prerelease is admitted only to the next channel', () => {
  assert.deepEqual(checkNpmVersionAvailability('0.4.0-rc.2', '["0.2.0","0.3.0"]\n'), {
    version: '0.4.0-rc.2',
    publishedVersions: 2,
    channel: 'next',
  })
...
test('published, stable, malformed, duplicate, and unbounded inventories fail closed', () => {
  assert.throws(
    () => checkNpmVersionAvailability('0.4.0-rc.2', '["0.3.0","0.4.0-rc.2"]'),
    /already published and immutable/,
  )
  assert.throws(() => checkNpmVersionAvailability('0.4.0', '["0.3.0"]'), /only.*prerelease/)
  assert.throws(() => checkNpmVersionAvailability('0.4.0-rc.2', '{'), /not JSON/)
  assert.throws(() => checkNpmVersionAvailability('0.4.0-rc.2', '[]'), /non-empty array/)
  assert.throws(() => checkNpmVersionAvailability('0.4.0-rc.2', '["0.3.0","0.3.0"]'), /repeats/)
  assert.throws(() => checkNpmVersionAvailability('0.4.0-rc.2', '[7]'), /non-string/)
  assert.throws(() => checkNpmVersionAvailability('0.4.0-rc.2', ' '.repeat(1024 * 1024 + 1)), /oversized/)
})
...
test('the CLI accepts only an absolute bounded regular inventory file', t => {
  ...
  assert.match(output, /0\.4\.0-rc\.2 is absent from 2 immutable versions and will use next/)
