...
test('benchmark modes are closed, disjoint, and cover comparable tools', () => {
  ...
  assert.equal(cold.measuredIterations, 20)
  assert.equal(warm.measuredIterations, 20)
  assert.equal(api.measuredIterations, 20)
  assert.equal(memory.measuredIterations, 20)
  assert.equal(throughput.measuredIterations, 20)
