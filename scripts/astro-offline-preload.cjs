...
const allowedBinaryInput = process.env.ZIGCSS_ASTRO_ALLOWED_BINARY
let allowedBinary = null
let allowedBinaryIdentity = null
if (allowedBinaryInput !== undefined) {
  if (
    typeof allowedBinaryInput !== 'string' || allowedBinaryInput.length === 0 ||
    !path.isAbsolute(allowedBinaryInput) || path.resolve(allowedBinaryInput) !== allowedBinaryInput ||
    Buffer.byteLength(allowedBinaryInput) > 4096 || /[\0\r\n]/.test(allowedBinaryInput)
  ) fail('allowed native binary must be a bounded normalized absolute path')
  ...
  if (path.relative(traceRoot, allowedBinaryInput) !== expectedRelative) {
    fail('allowed native binary must be the staged ZigCSS executable under the temporary project')
  }
  const stat = fs.lstatSync(allowedBinaryInput, { bigint: true })
  if (
    !stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1n || stat.size <= 0n ||
    stat.size > BigInt(maximumBinaryBytes) || fs.realpathSync(allowedBinaryInput) !== allowedBinaryInput
  ) {
    fail('allowed native binary must be a bounded canonical singly-linked regular non-symlink file')
  }
  if (process.platform !== 'win32' && (stat.mode & 0o111n) === 0n) {
    fail('allowed native binary must be executable')
  }
  allowedBinary = allowedBinaryInput
  allowedBinaryIdentity = Object.freeze({
...
if (allowedEsbuildInput !== undefined) {
  ...
  if (
    typeof allowedEsbuildInput !== 'string' || allowedEsbuildInput.length === 0 ||
    !path.isAbsolute(allowedEsbuildInput) || path.resolve(allowedEsbuildInput) !== allowedEsbuildInput ||
    Buffer.byteLength(allowedEsbuildInput) > 4096 || /[\0\r\n]/.test(allowedEsbuildInput)
  ) fail('allowed esbuild binary must be a bounded normalized absolute path')
  ...
  if (path.relative(traceRoot, allowedEsbuildInput) !== expectedRelative) {
    fail('allowed esbuild binary must use its exact platform-package path under the temporary project')
  }
  const stat = fs.lstatSync(allowedEsbuildInput, { bigint: true })
  if (
    !stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1n || stat.size <= 0n ||
    stat.size > BigInt(maximumBinaryBytes) || fs.realpathSync(allowedEsbuildInput) !== allowedEsbuildInput
  ) fail('allowed esbuild binary must be a bounded canonical singly-linked regular non-symlink file')
  if (process.platform !== 'win32' && (stat.mode & 0o111n) === 0n) fail('allowed esbuild binary must be executable')
  ...
  allowedEsbuild = allowedEsbuildInput
  allowedEsbuildIdentity = Object.freeze({
    ctimeNs: stat.ctimeNs,
    dev: stat.dev,
    ino: stat.ino,
    mode: stat.mode,
