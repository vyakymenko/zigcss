...
Dependency reporting recognizes decoded string/URL operands of top-level `@import` plus adapter-supplied external-composition facts. Results deeply own the decoded specifier and source name, retain authored span order and duplicates, and carry the operand span. Unsupported, malformed, or dynamic preludes are not guessed into dependencies. One combined count/owned-byte boundary reports `resource_limit` and discards partial facts. Stable CSS returns `module_exports = null`; a successful experimental CSS Modules compile transfers first-authored class/local-value entries plus deeply owned local/global/dependency composition references into the same result allocator. Errors and module-limit failures publish no partial export or dependency state.
