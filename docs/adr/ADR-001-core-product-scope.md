...
The LSP remains experimental while protocol stress/leak coverage and editor packaging are incomplete. Implemented features consume the rebuilt tokenizer, syntax tree, spans, diagnostics, and bounded open-document workspace indexes rather than a parallel textual parser. The public compile service remains disabled until compiler correctness and bounded process/request isolation both pass. Package-manager and native artifact claims remain gated on installation smoke tests.
