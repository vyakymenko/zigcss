...
1. The stable native promotion uses exact version `0.6.0` and one new protected tag, `v0.6.0`. The project must not move, delete, or recreate `v0.6.0-rc.2`; its GitHub prerelease, npm `next` package, assets, attestations, and ledger evidence remain historical facts.
...
4. The existing GitHub Pages deployment is the canonical package website. The owner authorizes deployment only when the exact `.github/workflows/build.yml` workflow named `Build` succeeds for the same-repository `main` commit that the documentation job checks out and builds. The site must ship a canonical URL, crawl policy, sitemap, route-specific indexable metadata, social metadata, structured software metadata, and a supported HTML meta CSP before any resource-bearing element. GitHub Pages owns the HTTP response headers: COOP, CORP, `Permissions-Policy`, `X-Frame-Options`, and CSP `frame-ancestors` are container-only guarantees and are not claimed for Pages. Search Console ownership tokens or account actions remain operator-owned and are not invented or committed.
...
8. This decision authorizes the repository branch push, exact-main integration, GitHub Pages deployment, one verified GitHub stable release, and immutable npm `latest` version required for `0.6.0` after their gates pass. The historical GitHub Release now reads back `immutable: false`; repository Immutable Releases applies only to future releases. This decision does not authorize Homebrew, editor-extension, container-registry, service, or unrelated infrastructure publication.
...
- Google indexing receives stable canonical pages, per-route robots directives, and `sitemap.xml`; the project owner can submit the sitemap after deployment without granting repository code access to Search Console. A GitHub Pages project site cannot control the origin-root `/robots.txt`, so the repository deliberately does not publish a misleading `/zigcss/robots.txt` file.
- Every generated Pages document carries the same exact meta-deliverable CSP subset. The optional static container adds response-only framing and cross-origin isolation controls plus a closed permissions policy; tests keep the two surfaces distinct so the project does not overstate controls that GitHub Pages does not expose.
