...
The project owner authorized an autonomous persistent goal with an explicit quality and execution constraint: development must use the model configured for the task, `gpt-5.6-sol` with ultra reasoning, and must not delegate implementation to subagents, child tasks, or a fallback model.
...
- Autonomous roadmap work uses `gpt-5.6-sol` with ultra reasoning as configured for the approved task.
