...
Within one `Context`, validators may reuse a successfully completed minified CSS plus source-map emission proof only for the exact same `RuleList` pointer. This relies on the immutable arena contract above: roots are never individually freed, address-reused, or mutated in place before `Compilation.deinit`. A distinct pointer always retains the two-output CSS/map differential comparison; a cache miss for an identical pointer still performs one complete proof emit. Executable counters bind a complete optimizer no-op and a modern target-prefix no-op to one proof emit each, while a changed math root requires one precondition emit plus both differential emissions.
