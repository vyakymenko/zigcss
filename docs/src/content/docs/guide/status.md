...
| LSP | Experimental | Bounded dynamic framing accepts sequential CRLF `Content-Length` messages with 8 KiB headers and bodies up to 16 MiB; JSON behavior, method lifecycle, positions, features, and the legacy parser remain later gates. |
...
The experimental LSP transport owns each request body independently, accepts sequential HTTP-style header blocks terminated by exact CRLF pairs, and exits cleanly at a frame boundary on EOF. Headers are bounded to 8 KiB and each dynamically allocated body to 16 MiB. Missing, duplicate, non-decimal, overflowing, oversized, malformed, or truncated `Content-Length` framing is rejected before JSON handling. A real child-process transcript covers a request above the former 8 KiB ceiling followed by a second request. Structured JSON-RPC errors, notification/request distinctions, shutdown/exit and document lifecycle, UTF-16 positions, compiler-diagnostic migration, and syntax-aware editor features remain owned by `LSP-002` through `LSP-007`.
