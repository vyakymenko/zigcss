...
`CompileOptions` currently admits only CSS, pretty/minified output, separate maps, the closed optimizer preset, the verified target-prefix pass with a borrowed canonical query, explicitly experimental trusted native plugins, and opt-in profiling. Targets without prefixing, prefixing without targets, forged queries, optimized source maps, and active plugin source maps return `API0001` diagnostics without CSS. An unavailable timer returns the operational `ProfilingUnavailable` error. Stable CSS returns no module exports. Remote package publication and CSS Modules remain later packages.
