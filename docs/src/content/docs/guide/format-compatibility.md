...
| CSS Modules | The FNV/legacy-AST parser is removed. `.css_modules` now walks the rebuilt typed AST, scopes local-by-default classes with source-specific SHA-256 names, and returns owned first-seen exports. | This is the exact [native CSS Modules subset](/guide/css-modules), not a claim of compatibility with the complete CSS Modules ecosystem. Deferred semantics fail with `CSS0009` and no CSS. |
...
CSS Modules is the one accepted limited-native direction. `MODULE-001` now provides library-only deterministic names, owned export mappings, limits, source maps, direct CSS import facts, recovery-disabled independent generated-CSS validation, and strict negative behavior. `MODULE-002` may add an explicitly bounded grammar for local/global scope, composition, values, and module dependencies. Those semantics remain errors until that package is complete.
