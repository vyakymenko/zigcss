...
| CSS Modules | The FNV/legacy-AST parser is removed. `.css_modules` walks the rebuilt typed AST, supports source-specific names and functional scope, and returns owned exports with local/global/external composition references plus module dependency facts. | This is the exact [native CSS Modules subset](/guide/css-modules), not a claim of compatibility with the complete CSS Modules ecosystem. It never loads external modules; deferred or ambiguous semantics fail with `CSS0009` and no CSS. |
...
CSS Modules is the one accepted limited-native direction. `MODULE-001` provides library-only deterministic names, owned export mappings, limits, source maps, direct CSS import facts, recovery-disabled independent generated-CSS validation, and strict negative behavior. `MODULE-002` adds occurrence-keyed functional scope and a plain-class composition grammar with owned references and combined dependency limits. Local values remain the final bounded slice; raw ICSS import/export and implicit external resolution stay unavailable.
