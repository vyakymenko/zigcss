...
| CSS Modules | The FNV/legacy-AST parser is removed. `.css_modules` now walks the rebuilt typed AST, scopes local-by-default classes with source-specific SHA-256 names, supports occurrence-sensitive functional local/global wrappers, and returns owned first-seen exports. | This is the exact [native CSS Modules subset](/guide/css-modules), not a claim of compatibility with the complete CSS Modules ecosystem. Deferred or ambiguous semantics fail with `CSS0009` and no CSS. |
...
CSS Modules is the one accepted limited-native direction. `MODULE-001` provides library-only deterministic names, owned export mappings, limits, source maps, direct CSS import facts, recovery-disabled independent generated-CSS validation, and strict negative behavior. The first `MODULE-002` slice adds functional `:local(...)`/`:global(...)` scope with occurrence-keyed emission while keeping ambiguous bare modes strict. Composition, values, and module dependencies remain errors until their bounded slices complete.
