# Current source CSS grammar compatibility

This is the tested grammar boundary of the current unpublished `0.7.0-rc.1` recovery compiler. It is not a claim of complete browser compatibility, property-value validation, or production readiness. Published stable 0.6.0 predates this parser contract: its CLI rejects `--autoprefix`/`--browsers` and CSS `--source-map`, and it does not provide the current all-syntax optimizer, mapping, or hardened-output guarantees. Unless a paragraph explicitly says stable, every CLI capability below requires a current checkout and freshly built `zig-out/bin/zigcss`.
...
## Current-source verified optimizer composition

The current recovery CLI's `--optimize` flag invokes one closed seven-pass preset over verified order-preserving analysis, cleanup, value, declaration, and rule passes. CSS enters that core directly; SCSS, indented Sass, Less, and Stylus run the identical preset only after their native frontend has produced complete recovery-disabled CSS. Compatibility rewriting, extraction, experimental passes, custom-property resolution, logical-to-physical conversion, and reorder authority are excluded. An exact whole-plan no-op returns before fixed-point emission. When proof-carrying rewrites expose another candidate, execution is bounded to 32 parse-transform-emit rounds and succeeds at an exact no-op root or when two consecutive emitted byte sequences match.
...
Current-source map generation exists for ordinary library parse/pass/emit pipelines. The current source-built CLI embeds that map deterministically for native CSS files, stdin, watch, and bounded batches. The four self-contained native preprocessor frontends compose their mappings through the same core before the identical inline policy is applied. Fixed-point optimization is available for all five syntaxes but remains incompatible with source maps; the independently configured target-prefix pass can either emit composed mappings without optimization or run after an unmapped optimizer fixed point. Extraction flags, CSS-in-JS, the [experimental library-only CSS Modules subset](/guide/css-modules), and the public compile service remain outside the native CSS grammar/CLI contract. SCSS, Sass, Less, and Stylus have their own exact [format compatibility contract](/guide/format-compatibility) before generated CSS reaches this grammar.
