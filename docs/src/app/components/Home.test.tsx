...
describe('Home', () => {
  ...
  it('opens as a terminal manifesto with an unavoidable release boundary', () => {
    ...
    expect(screen.getByText(/0\.6\.0-rc\.1.*experimental.*evaluate before production/i)).toBeInTheDocument()
    expect(screen.getByText(/selected 0\.6\.0-rc\.1 source candidate remains unpublished pending hosted release evidence/i)).toBeInTheDocument()
