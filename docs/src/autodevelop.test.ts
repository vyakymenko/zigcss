...
describe('autonomous development operations', () => {
  ...
  test('protocol preserves the approved authority and package loop', () => {
    ...
    expect(protocol).toContain('`NATIVE-001` through `NATIVE-005`')
    expect(protocol).toContain('`NSASS-010` through `NSASS-012`')
    expect(protocol).toContain('`NLESS-010` through `NLESS-012`')
    expect(protocol).toContain('`NSTYLUS-010` through `NSTYLUS-012`')
    expect(protocol).toContain('`NATIVE-006` through `NATIVE-009`')
    expect(protocol).toContain('must not block dependency-eligible native frontend correctness work')
    expect(protocol).toContain("owner's 2026-07-27 publication authorization")
    expect(protocol).toContain('GitHub prerelease and npm `next` evidence')
    expect(protocol).toContain('It may not publish `0.5.0-rc.1`')
  ...
  test('single-lane supervisor pins the model and passes hermetic control tests', () => {
    ...
    expect(after).toBe(before)
    expect(selftest).toContain('PASS=59 FAIL=0')
    ...
    expect(prompt).toContain('NLESS-010 through NLESS-012')
    expect(prompt).toContain('NSTYLUS-010 through NSTYLUS-012')
    expect(prompt).toContain('earliest dependency-eligible package not marked VERIFIED')
    expect(prompt).toContain('NATIVE-006 through NATIVE-009')
    ...
    expect(prompt).toContain('only the first fully graduated native release')
    expect(prompt).toContain('GitHub prerelease plus npm `next` publication evidence')
    expect(prompt).toContain('Never publish the provider-backed `0.5.0-rc.1`')
