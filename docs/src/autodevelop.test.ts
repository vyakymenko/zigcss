...
describe('autonomous development operations', () => {
  ...
  test('protocol preserves the approved authority and package loop', () => {
    ...
    expect(protocol).toContain('Milestone 10 and `NATIVE-009` are verified')
    expect(protocol).toContain('`REL-010` stable-promotion sequence')
    expect(protocol).toContain('must not block the dependency-eligible `REL-010` work')
  ...
  test('single-lane supervisor pins the model and passes hermetic control tests', () => {
    ...
    expect(prompt).toContain('Milestone 10 and NATIVE-009 are verified')
    expect(prompt).toContain('REL-010 stable-promotion sequence')
    expect(prompt).toContain('earliest dependency-eligible REL-010 slice not marked VERIFIED')
    expect(prompt).toContain('pending external BENCH-007 runner must not block REL-010')
    expect(prompt).toContain('Do not push ordinary branch checkpoints from this model pass')
    expect(prompt).toContain('exact stable `v0.6.0` promotion')
    expect(prompt).toContain('non-prerelease GitHub Release, npm `latest`, and the existing GitHub Pages deployment')
    expect(prompt).toContain('Never move or recreate `v0.6.0-rc.2`')
