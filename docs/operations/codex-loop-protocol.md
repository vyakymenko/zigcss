...
This is the repository-owned operating procedure for executing `DEVELOPMENT_PLAN.md`. It adapts Alvo's protocol/orientation split to ZigCSS, its persistent Codex goal, and its strict single-agent and guarded-publication authority boundary.
...
- ordinary model passes cannot push; the outer supervisor alone performs a non-interactive, five-minute-bounded non-force push of every green `HEAD` to the current `vale/*` recovery branch on the closed approved `origin`, reads back its exact object ID, and integrates `main` atomically with that branch after four passes, for the exact closed `NATIVE-007` hosted-validation terminal, or immediately for completion/release validation; it stops on rejection, timeout, drift, or mismatch;
