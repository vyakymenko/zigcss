...
The `v0.5.0-rc.1` tag must point at the exact integrated `main` commit whose synchronized version, release metadata, consumers, native targets, and workflow policy are green.
...
4. fails if `0.5.0-rc.1` already exists or the registry response is malformed.
