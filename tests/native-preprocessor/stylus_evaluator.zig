...
const regression_432_literal_call_input =
    \\body
    \\  foo: unit(16 / 4, px)
    \\  foo: foo(16 / 4)
    \\  foo: (16/4)px
...
const regression_480_expression_multiplication_input =
    \\n = 5px
    \\
    \\body
    \\  foo: n * 5
...
const regression_499_complex_grouped_extension_input =
    "\n" ++
    \\ul
    \\  li
    \\    a
...
const regression_499_complex_grouped_extension_css =
    "ul li a,a.button,input[type=submit]{color:#00f}" ++
    "html body h1,html body .title,html body .huge-title{" ++
...
const conditional_descendant_class_selector_input =
    \\if true
    \\  tag .class
    \\  bar
    \\    foo bar
...
const conditional_descendant_class_selector_css = "tag .class,bar{foo:bar}";
...
const color_identifier_boundary_input =
    \\#fea-ca
    \\  padding 5px
    \\
    \\#ffffff
...
test "native Stylus literal calls own the finite regression 432 argument contract" {
    const lower_input =
        \\body
        \\  value foo(8 / 2)
    ;
    ...
    var lower = try compile(std.testing.allocator, lower_input, lower_limits);
    defer lower.deinit();
    try std.testing.expectEqualStrings("body{value:foo(4)}", lower.css());

    var terminal_limits = stylus_evaluator.Limits{};
    terminal_limits.max_source_bytes = regression_432_literal_call_input.len;
    terminal_limits.max_selectors = 1;
    var first = try compile(
        std.testing.allocator,
        regression_432_literal_call_input,
        terminal_limits,
    );
    defer first.deinit();
    var second = try compile(
    ...
    try std.testing.expectEqualStrings(regression_432_literal_call_css, first.css());
    try std.testing.expectEqualStrings(first.css(), second.css());
    try std.testing.expectEqualSlices(u8, first.sourceMap().?, second.sourceMap().?);
    try std.testing.expectEqual(@as(usize, 0), first.nativeDiagnostics().len);
    try std.testing.expectEqual(@as(usize, 0), first.coreDiagnostics().len);
...
test "native Stylus expression multiplication owns the finite regression 480 contract" {
    const lower_input =
        \\n = 5em 5px
        \\body
        \\  foo n * 2
    ...
    var lower = try compile(std.testing.allocator, lower_input, lower_limits);
    defer lower.deinit();
    try std.testing.expectEqualStrings("body{foo:10em;foo:10em}", lower.css());

    var terminal_limits = stylus_evaluator.Limits{};
    terminal_limits.max_source_bytes = regression_480_expression_multiplication_input.len;
    terminal_limits.max_selectors = 2;
    var first = try compile(
        std.testing.allocator,
        regression_480_expression_multiplication_input,
        terminal_limits,
    );
    defer first.deinit();
    var second = try compile(
    ...
    try std.testing.expectEqualStrings(regression_480_expression_multiplication_css, first.css());
    try std.testing.expectEqualStrings(first.css(), second.css());
    try std.testing.expectEqualSlices(u8, first.sourceMap().?, second.sourceMap().?);
    try std.testing.expectEqual(@as(usize, 0), first.nativeDiagnostics().len);
    try std.testing.expectEqual(@as(usize, 0), first.coreDiagnostics().len);
...
test "native Stylus complex grouped extensions own the finite regression 499 contract" {
    const lower_input =
        \\ul
        \\  li
        \\    a
    ...
    var terminal_limits = stylus_evaluator.Limits{};
    terminal_limits.max_source_bytes = regression_499_complex_grouped_extension_input.len;
    terminal_limits.max_selectors = 15;
    var first = try compile(
        std.testing.allocator,
        regression_499_complex_grouped_extension_input,
        terminal_limits,
    );
    defer first.deinit();
    var second = try compile(
    ...
    try std.testing.expectEqualStrings(regression_499_complex_grouped_extension_css, first.css());
    try std.testing.expectEqualStrings(first.css(), second.css());
    try std.testing.expectEqualSlices(u8, first.sourceMap().?, second.sourceMap().?);
    try std.testing.expectEqual(@as(usize, 0), first.nativeDiagnostics().len);
    try std.testing.expectEqual(@as(usize, 0), first.coreDiagnostics().len);
...
test "native Stylus conditional descendant-class selectors own the finite nested group contract" {
    var terminal_limits = stylus_evaluator.Limits{};
    terminal_limits.max_source_bytes = conditional_descendant_class_selector_input.len;
    terminal_limits.max_selectors = 2;
    var first = try compile(
        std.testing.allocator,
        conditional_descendant_class_selector_input,
        terminal_limits,
    );
    defer first.deinit();
    var second = try compile(
    ...
    try std.testing.expectEqualStrings(conditional_descendant_class_selector_css, first.css());
    try std.testing.expectEqualStrings(first.css(), second.css());
    try std.testing.expectEqualSlices(u8, first.sourceMap().?, second.sourceMap().?);
    try std.testing.expectEqual(@as(usize, 0), first.nativeDiagnostics().len);
    try std.testing.expectEqual(@as(usize, 0), first.coreDiagnostics().len);
    ...
    try expectSemanticRejectionWithLimits(
        ...
        .resource_limit,
        "native Stylus selector limit exceeded",
        @intCast(std.mem.indexOf(u8, conditional_descendant_class_selector_input, "tag .class").?),
    );
}
...
test "native Stylus color tokens keep finite selector and expression boundaries" {
    const lower_input =
        \\#fea-ca
        \\  foo #fea
    ;
    ...
    var lower = try compile(std.testing.allocator, lower_input, lower_limits);
    defer lower.deinit();
    try std.testing.expectEqualStrings("#fea-ca{foo:#fea}", lower.css());

    var terminal_limits = stylus_evaluator.Limits{};
    terminal_limits.values.max_collection_items = 4;
    var first = try compile(
        std.testing.allocator,
        color_identifier_boundary_input,
        terminal_limits,
    );
    defer first.deinit();
    var second = try compile(
    ...
    try std.testing.expectEqualStrings(color_identifier_boundary_css, first.css());
    try std.testing.expectEqualStrings(first.css(), second.css());
    try std.testing.expectEqualSlices(u8, first.sourceMap().?, second.sourceMap().?);
    try std.testing.expectEqual(@as(usize, 0), first.nativeDiagnostics().len);
    try std.testing.expectEqual(@as(usize, 0), first.coreDiagnostics().len);
    ...
    try expectSemanticRejectionWithLimits(
        ...
        .resource_limit,
        "native Stylus value limit exceeded",
        @intCast(std.mem.lastIndexOf(u8, color_identifier_boundary_input, "#fea-ca").?),
    );
}
...
fn exerciseColorIdentifierBoundaryAllocationFailures(allocator: std.mem.Allocator) !void {
    var limits = stylus_evaluator.Limits{};
    limits.values.max_collection_items = 4;
    var result = try compile(allocator, color_identifier_boundary_input, limits);
    defer result.deinit();
...
fn exerciseRegression432LiteralCallAllocationFailures(allocator: std.mem.Allocator) !void {
    var limits = stylus_evaluator.Limits{};
    limits.max_source_bytes = regression_432_literal_call_input.len;
    limits.max_selectors = 1;
    var result = try compile(allocator, regression_432_literal_call_input, limits);
    defer result.deinit();
...
fn exerciseRegression480ExpressionMultiplicationAllocationFailures(allocator: std.mem.Allocator) !void {
    var limits = stylus_evaluator.Limits{};
    limits.max_source_bytes = regression_480_expression_multiplication_input.len;
    limits.max_selectors = 2;
    var result = try compile(allocator, regression_480_expression_multiplication_input, limits);
    defer result.deinit();
...
fn exerciseRegression499ComplexGroupedExtensionAllocationFailures(allocator: std.mem.Allocator) !void {
    var limits = stylus_evaluator.Limits{};
    limits.max_source_bytes = regression_499_complex_grouped_extension_input.len;
    limits.max_selectors = 15;
    var result = try compile(allocator, regression_499_complex_grouped_extension_input, limits);
    defer result.deinit();
...
fn exerciseConditionalDescendantClassSelectorAllocationFailures(
    allocator: std.mem.Allocator,
) !void {
    var limits = stylus_evaluator.Limits{};
    limits.max_source_bytes = conditional_descendant_class_selector_input.len;
    limits.max_selectors = 2;
    var result = try compile(allocator, conditional_descendant_class_selector_input, limits);
    defer result.deinit();
    try std.testing.expectEqualStrings(conditional_descendant_class_selector_css, result.css());
}
...
test "native Stylus color identifier boundaries handle every allocation failure" {
    try std.testing.checkAllAllocationFailures(
        std.testing.allocator,
...
test "native Stylus regression 432 literal calls handle every allocation failure" {
    try std.testing.checkAllAllocationFailures(
        std.testing.allocator,
...
test "native Stylus regression 480 expression multiplication handles every allocation failure" {
    try std.testing.checkAllAllocationFailures(
        std.testing.allocator,
...
test "native Stylus regression 499 complex grouped extensions handle every allocation failure" {
    try std.testing.checkAllAllocationFailures(
        std.testing.allocator,
...
test "native Stylus conditional descendant-class selector groups handle every allocation failure" {
    try std.testing.checkAllAllocationFailures(
        std.testing.allocator,
        exerciseConditionalDescendantClassSelectorAllocationFailures,
        .{},
    );
}
