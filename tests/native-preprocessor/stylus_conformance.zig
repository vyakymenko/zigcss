...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    try std.testing.expectEqual(@as(usize, 183), exact_success_count);
    try std.testing.expectEqual(@as(usize, 143), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x1b0e50ebf29bd9a9), exact_case_id_hash.final());
}
