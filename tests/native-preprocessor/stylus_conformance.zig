...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    try std.testing.expectEqual(@as(usize, 199), exact_success_count);
    try std.testing.expectEqual(@as(usize, 127), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0xcdcf18d0846207e7), exact_case_id_hash.final());
}
