...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_keyframe_newlines = std.mem.eql(
                u8,
                case.id,
                "stylus-official-keyframes-newlines",
            );
            if (!became_exact_with_keyframe_newlines) {
    ...
    if (exact_success_count != 268 or nonconforming_count != 58 or
        exact_hash != 0xc463f9900345eb3c)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 268), exact_success_count);
    try std.testing.expectEqual(@as(usize, 58), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0xc463f9900345eb3c), exact_hash);
    try std.testing.expectEqual(
        @as(u64, 0x4b4f42e6323e1063),
