...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            if (!std.mem.eql(u8, case.id, "stylus-official-mixins-complex")) {
            ...
            const became_exact_with_media_complex =
                ...
                std.mem.eql(u8, case.id, "stylus-official-mixins-complex");
    ...
    if (exact_success_count != 276 or nonconforming_count != 50 or
        exact_hash != 0xb61c2417bc249721)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 276), exact_success_count);
    try std.testing.expectEqual(@as(usize, 50), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0xb61c2417bc249721), exact_hash);
