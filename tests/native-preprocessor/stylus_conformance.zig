...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    try std.testing.expectEqual(@as(usize, 200), exact_success_count);
    try std.testing.expectEqual(@as(usize, 126), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0xbbf2da7afd8ab2ce), exact_case_id_hash.final());
}
