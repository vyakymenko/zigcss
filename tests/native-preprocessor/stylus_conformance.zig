...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_keyframe_defaults = std.mem.eql(
                u8,
                case.id,
                "stylus-official-keyframes-fabrication-defaults",
            );
            if (!became_exact_with_keyframe_defaults) {
    ...
    if (exact_success_count != 267 or nonconforming_count != 59 or
        exact_hash != 0x4b4f42e6323e1063)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 267), exact_success_count);
    try std.testing.expectEqual(@as(usize, 59), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x4b4f42e6323e1063), exact_hash);
    try std.testing.expectEqual(
        @as(u64, 0xe725f9c4f00ffe2c),
