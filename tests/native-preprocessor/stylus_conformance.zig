...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_multiple_definition_extension = std.mem.eql(
                u8,
                case.id,
                "stylus-official-extend-multiple-definitions",
            );
            if (!became_exact_with_multiple_definition_extension) {
    ...
    if (exact_success_count != 232 or nonconforming_count != 94 or
        exact_hash != 0x1396e7ad27da47d2)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 232), exact_success_count);
    try std.testing.expectEqual(@as(usize, 94), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x1396e7ad27da47d2), exact_hash);
    try std.testing.expectEqual(
        @as(u64, 0x4841579bd34469de),
