...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_css_large =
                std.mem.eql(u8, case.id, "stylus-official-css-large") or
                std.mem.eql(u8, case.id, "stylus-official-mixins-reset") or
                std.mem.eql(u8, case.id, "stylus-official-parent") or
                std.mem.eql(u8, case.id, "stylus-official-reset");
            if (!became_exact_with_css_large) {
    ...
    if (exact_success_count != 219 or nonconforming_count != 107 or
        exact_hash != 0x5e87a9d2dfa920cd)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 219), exact_success_count);
    try std.testing.expectEqual(@as(usize, 107), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x5e87a9d2dfa920cd), exact_hash);
    try std.testing.expectEqual(
        @as(u64, 0x8dbe94d06252fe79),
