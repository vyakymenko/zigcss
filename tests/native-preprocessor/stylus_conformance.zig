...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    try std.testing.expectEqual(@as(usize, 204), exact_success_count);
    try std.testing.expectEqual(@as(usize, 122), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x8e2ca57648ec57b0), exact_case_id_hash.final());
}
