...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_complex_extension = std.mem.eql(
                u8,
                case.id,
                "stylus-official-extend-complex",
            );
            if (!became_exact_with_complex_extension) {
    ...
    if (exact_success_count != 226 or nonconforming_count != 100 or
        exact_hash != 0xfe3d6ca881fc5226)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 226), exact_success_count);
    try std.testing.expectEqual(@as(usize, 100), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0xfe3d6ca881fc5226), exact_hash);
    try std.testing.expectEqual(
        @as(u64, 0x93598f2519b102bd),
        previous_exact_case_id_hash.final(),
    );
    try std.testing.expectEqualStrings(
        "stylus-official-extend-in-loop",
