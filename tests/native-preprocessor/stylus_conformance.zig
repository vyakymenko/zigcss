...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_nested_mixin_extension = std.mem.eql(
                u8,
                case.id,
                "stylus-official-extend-in-mixin-nested",
            );
            if (!became_exact_with_nested_mixin_extension) {
    ...
    if (exact_success_count != 231 or nonconforming_count != 95 or
        exact_hash != 0x4841579bd34469de)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 231), exact_success_count);
    try std.testing.expectEqual(@as(usize, 95), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x4841579bd34469de), exact_hash);
    try std.testing.expectEqual(
        @as(u64, 0x5f6e3c393b48fdd9),
