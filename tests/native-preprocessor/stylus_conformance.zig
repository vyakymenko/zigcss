...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    if (exact_success_count != 319 or nonconforming_count != 7 or
        exact_hash != 0xa77f68a47fde66f7)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 319), exact_success_count);
    try std.testing.expectEqual(@as(usize, 7), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0xa77f68a47fde66f7), exact_hash);
