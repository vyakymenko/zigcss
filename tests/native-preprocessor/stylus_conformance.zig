...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    try std.testing.expectEqual(@as(usize, 186), exact_success_count);
    try std.testing.expectEqual(@as(usize, 140), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x982bd82e665c42ed), exact_case_id_hash.final());
}
