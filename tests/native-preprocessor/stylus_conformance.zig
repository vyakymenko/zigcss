...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_nested_function =
                std.mem.eql(u8, case.id, "stylus-official-functions-nested");
            if (!became_exact_with_nested_function) {
    ...
    if (exact_success_count != 249 or nonconforming_count != 77 or
        exact_hash != 0x5d596113f879443e)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 249), exact_success_count);
    try std.testing.expectEqual(@as(usize, 77), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x5d596113f879443e), exact_hash);
    try std.testing.expectEqual(
        @as(u64, 0x87845eab4c56f843),
        previous_exact_case_id_hash.final(),
    );
    try std.testing.expectEqualStrings(
        "stylus-official-functions-property",
