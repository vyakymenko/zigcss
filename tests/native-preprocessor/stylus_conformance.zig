...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    if (exact_success_count != 309 or nonconforming_count != 17 or
        exact_hash != 0x3bd0472031f4f0df)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 309), exact_success_count);
    try std.testing.expectEqual(@as(usize, 17), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x3bd0472031f4f0df), exact_hash);
