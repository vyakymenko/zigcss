...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_multiline =
                ...
                std.mem.eql(u8, case.id, "stylus-official-regression-839");
    ...
    if (exact_success_count != 278 or nonconforming_count != 48 or
        exact_hash != 0xecd638eb43f8a071)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 278), exact_success_count);
    try std.testing.expectEqual(@as(usize, 48), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0xecd638eb43f8a071), exact_hash);
    ...
    try std.testing.expectEqualStrings(
        "stylus-official-object-complex",
