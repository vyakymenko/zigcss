...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_literal_color = std.mem.eql(
                u8,
                case.id,
                "stylus-official-literal-color",
            );
            if (!became_exact_with_literal_color) {
    ...
    if (exact_success_count != 271 or nonconforming_count != 55 or
        exact_hash != 0x9c8106b53217cf87)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 271), exact_success_count);
    try std.testing.expectEqual(@as(usize, 55), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x9c8106b53217cf87), exact_hash);
    try std.testing.expectEqual(
        @as(u64, 0x5da14a9eda4fc533),
        prior_exact_case_id_hash.final(),
    );
    try std.testing.expectEqualStrings(
        "stylus-official-media-bubble",
