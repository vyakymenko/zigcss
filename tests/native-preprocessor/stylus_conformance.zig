...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    try std.testing.expectEqual(@as(usize, 166), exact_success_count);
    try std.testing.expectEqual(@as(usize, 160), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0xcf1a5eefdb5f2119), exact_case_id_hash.final());
}
