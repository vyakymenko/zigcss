...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_if_mixin =
                std.mem.eql(u8, case.id, "stylus-official-if-mixin");
            if (!became_exact_with_if_mixin) {
    ...
    if (exact_success_count != 254 or nonconforming_count != 72 or
        exact_hash != 0xde166a5184d2235c)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 254), exact_success_count);
    try std.testing.expectEqual(@as(usize, 72), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0xde166a5184d2235c), exact_hash);
    try std.testing.expectEqual(
        @as(u64, 0xa144ba46615b1433),
        previous_exact_case_id_hash.final(),
    );
    try std.testing.expectEqualStrings(
        "stylus-official-import-clone",
