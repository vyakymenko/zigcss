...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_multiple_selector_extension = std.mem.eql(
                u8,
                case.id,
                "stylus-official-extend-multiple-selectors",
            );
            if (!became_exact_with_multiple_selector_extension) {
    ...
    if (exact_success_count != 233 or nonconforming_count != 93 or
        exact_hash != 0x62cf4c87c93727f3)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 233), exact_success_count);
    try std.testing.expectEqual(@as(usize, 93), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x62cf4c87c93727f3), exact_hash);
    try std.testing.expectEqual(
        @as(u64, 0x1396e7ad27da47d2),
