...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    if (exact_success_count != 306 or nonconforming_count != 20 or
        exact_hash != 0x10c454cff99e1f65)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 306), exact_success_count);
    try std.testing.expectEqual(@as(usize, 20), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x10c454cff99e1f65), exact_hash);
