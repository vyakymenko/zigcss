...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_eol_escape = std.mem.eql(
                u8,
                case.id,
                "stylus-official-eol-escape",
            );
            if (!became_exact_with_eol_escape) {
    ...
    if (exact_success_count != 225 or nonconforming_count != 101 or
        exact_hash != 0x93598f2519b102bd)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 225), exact_success_count);
    try std.testing.expectEqual(@as(usize, 101), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x93598f2519b102bd), exact_hash);
    try std.testing.expectEqual(
        @as(u64, 0x28a3bf9090c61499),
