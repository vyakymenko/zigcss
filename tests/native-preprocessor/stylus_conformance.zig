...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    var pre_variables_case_id_hash = std.hash.Wyhash.init(0);
    var pre_supports_case_id_hash = std.hash.Wyhash.init(0);
    var pre_selectors_nested_case_id_hash = std.hash.Wyhash.init(0);
    var pre_selectors_complex_case_id_hash = std.hash.Wyhash.init(0);
    ...
    var pre_regression_475_case_id_hash = std.hash.Wyhash.init(0);
    var pre_regression_432_case_id_hash = std.hash.Wyhash.init(0);
    var pre_regression_2820_case_id_hash = std.hash.Wyhash.init(0);
    var pre_regression_274_case_id_hash = std.hash.Wyhash.init(0);
    var pre_regression_272_case_id_hash = std.hash.Wyhash.init(0);
    var pre_regression_216_case_id_hash = std.hash.Wyhash.init(0);
    var pre_regression_1997_case_id_hash = std.hash.Wyhash.init(0);
    var pre_regression_1584_case_id_hash = std.hash.Wyhash.init(0);
    var pre_regression_1572_case_id_hash = std.hash.Wyhash.init(0);
    var pre_regression_156_case_id_hash = std.hash.Wyhash.init(0);
    var pre_regression_142_case_id_hash = std.hash.Wyhash.init(0);
    var pre_regression_137_case_id_hash = std.hash.Wyhash.init(0);
    var pre_regression_131_case_id_hash = std.hash.Wyhash.init(0);
    var pre_regression_130_case_id_hash = std.hash.Wyhash.init(0);
    var pre_regression_1277_case_id_hash = std.hash.Wyhash.init(0);
    var pre_regression_1206_case_id_hash = std.hash.Wyhash.init(0);
    var pre_regression_1205_case_id_hash = std.hash.Wyhash.init(0);
    var pre_regression_1182_case_id_hash = std.hash.Wyhash.init(0);
    ...
    var pre_subscript_range_case_id_hash = std.hash.Wyhash.init(0);
    var pre_subscript_assignment_case_id_hash = std.hash.Wyhash.init(0);
    ...
    var pre_complex_operators_case_id_hash = std.hash.Wyhash.init(0);
    var pre_operators_case_id_hash = std.hash.Wyhash.init(0);
    var pre_operator_range_case_id_hash = std.hash.Wyhash.init(0);
    var pre_object_mixin_case_id_hash = std.hash.Wyhash.init(0);
    var pre_object_complex_case_id_hash = std.hash.Wyhash.init(0);
    var pre_multiline_case_id_hash = std.hash.Wyhash.init(0);
    var pre_mixins_complex_case_id_hash = std.hash.Wyhash.init(0);
    var pre_media_complex_case_id_hash = std.hash.Wyhash.init(0);
    var pre_media_bubble_case_id_hash = std.hash.Wyhash.init(0);
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const is_variables = std.mem.eql(
                u8,
                case.id,
                "stylus-official-variables",
            );
            if (!is_variables) {
                target_exclusion_case_id_hash.update(case.id);
                target_exclusion_case_id_hash.update("\x00");
            }
            const became_exact_with_variables = is_variables;
            if (!became_exact_with_variables) {
                prior_exact_case_id_hash.update(case.id);
                prior_exact_case_id_hash.update("\x00");
            }
            const became_exact_with_supports = std.mem.eql(
                u8,
                case.id,
                "stylus-official-supports",
            ) or became_exact_with_variables;
            if (!became_exact_with_supports) {
                pre_variables_case_id_hash.update(case.id);
                pre_variables_case_id_hash.update("\x00");
            }
            const became_exact_with_selectors_nested = std.mem.eql(
                u8,
                case.id,
                "stylus-official-selectors-nested",
            ) or became_exact_with_supports;
            if (!became_exact_with_selectors_nested) {
                pre_supports_case_id_hash.update(case.id);
                pre_supports_case_id_hash.update("\x00");
            }
            const became_exact_with_selectors_complex =
                std.mem.eql(u8, case.id, "stylus-official-selectors-complex") or
                became_exact_with_selectors_nested;
            if (!became_exact_with_selectors_complex) {
                pre_selectors_nested_case_id_hash.update(case.id);
                pre_selectors_nested_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_503 =
                std.mem.eql(u8, case.id, "stylus-official-regression-503") or
                std.mem.eql(u8, case.id, "stylus-official-regression-504") or
                became_exact_with_selectors_complex;
            if (!became_exact_with_regression_503) {
                pre_selectors_complex_case_id_hash.update(case.id);
                pre_selectors_complex_case_id_hash.update("\x00");
            }
            ...
            const became_exact_with_regression_498 = std.mem.eql(
                u8,
                case.id,
                "stylus-official-regression-498",
            ) or became_exact_with_regression_499;
            if (!became_exact_with_regression_498) {
                pre_regression_499_case_id_hash.update(case.id);
                pre_regression_499_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_480 = std.mem.eql(
                u8,
                case.id,
                "stylus-official-regression-480",
            ) or became_exact_with_regression_498;
            if (!became_exact_with_regression_480) {
                pre_regression_498_case_id_hash.update(case.id);
                pre_regression_498_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_475 = std.mem.eql(
                u8,
                case.id,
                "stylus-official-regression-475",
            ) or became_exact_with_regression_480;
            if (!became_exact_with_regression_475) {
                pre_regression_480_case_id_hash.update(case.id);
                pre_regression_480_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_432 = std.mem.eql(
                u8,
                case.id,
                "stylus-official-regression-432",
            ) or became_exact_with_regression_475;
            if (!became_exact_with_regression_432) {
                pre_regression_475_case_id_hash.update(case.id);
                pre_regression_475_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_2820 = std.mem.eql(
                u8,
                case.id,
                "stylus-official-regression-2820",
            ) or became_exact_with_regression_432;
            if (!became_exact_with_regression_2820) {
                pre_regression_432_case_id_hash.update(case.id);
                pre_regression_432_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_274 = std.mem.eql(
                u8,
                case.id,
                "stylus-official-regression-274",
            ) or became_exact_with_regression_2820;
            if (!became_exact_with_regression_274) {
                pre_regression_2820_case_id_hash.update(case.id);
                pre_regression_2820_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_272 = std.mem.eql(
                u8,
                case.id,
                "stylus-official-regression-272",
            ) or became_exact_with_regression_274;
            if (!became_exact_with_regression_272) {
                pre_regression_274_case_id_hash.update(case.id);
                pre_regression_274_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_216 = std.mem.eql(
                u8,
                case.id,
                "stylus-official-regression-216",
            ) or became_exact_with_regression_272;
            if (!became_exact_with_regression_216) {
                pre_regression_272_case_id_hash.update(case.id);
                pre_regression_272_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_1997 = std.mem.eql(
                u8,
                case.id,
                "stylus-official-regression-1997",
            ) or became_exact_with_regression_216;
            if (!became_exact_with_regression_1997) {
                pre_regression_216_case_id_hash.update(case.id);
                pre_regression_216_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_1584 = std.mem.eql(
                u8,
                case.id,
                "stylus-official-regression-1584",
            ) or became_exact_with_regression_1997;
            if (!became_exact_with_regression_1584) {
                pre_regression_1997_case_id_hash.update(case.id);
                pre_regression_1997_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_1572 = std.mem.eql(
                u8,
                case.id,
                "stylus-official-regression-1572",
            ) or became_exact_with_regression_1584;
            if (!became_exact_with_regression_1572) {
                pre_regression_1584_case_id_hash.update(case.id);
                pre_regression_1584_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_156 = std.mem.eql(
                u8,
                case.id,
                "stylus-official-regression-156",
            ) or became_exact_with_regression_1572;
            if (!became_exact_with_regression_156) {
                pre_regression_1572_case_id_hash.update(case.id);
                pre_regression_1572_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_142 = std.mem.eql(
                u8,
                case.id,
                "stylus-official-regression-142",
            ) or became_exact_with_regression_156;
            if (!became_exact_with_regression_142) {
                pre_regression_156_case_id_hash.update(case.id);
                pre_regression_156_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_137 = std.mem.eql(
                u8,
                case.id,
                "stylus-official-regression-137",
            ) or became_exact_with_regression_142;
            if (!became_exact_with_regression_137) {
                pre_regression_142_case_id_hash.update(case.id);
                pre_regression_142_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_131 = std.mem.eql(
                u8,
                case.id,
                "stylus-official-regression-131",
            ) or became_exact_with_regression_137;
            if (!became_exact_with_regression_131) {
                pre_regression_137_case_id_hash.update(case.id);
                pre_regression_137_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_130 =
                std.mem.eql(u8, case.id, "stylus-official-regression-130") or
                std.mem.eql(u8, case.id, "stylus-official-regression-1998") or
                became_exact_with_regression_131;
            if (!became_exact_with_regression_130) {
                pre_regression_131_case_id_hash.update(case.id);
                pre_regression_131_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_1277 =
                std.mem.eql(u8, case.id, "stylus-official-regression-1277") or
                std.mem.eql(u8, case.id, "stylus-official-regression-1571") or
                became_exact_with_regression_130;
            if (!became_exact_with_regression_1277) {
                pre_regression_130_case_id_hash.update(case.id);
                pre_regression_130_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_1206 = std.mem.eql(
                u8,
                case.id,
                "stylus-official-regression-1206",
            ) or became_exact_with_regression_1277;
            if (!became_exact_with_regression_1206) {
                pre_regression_1277_case_id_hash.update(case.id);
                pre_regression_1277_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_1205 = std.mem.eql(
                u8,
                case.id,
                "stylus-official-regression-1205",
            ) or became_exact_with_regression_1206;
            if (!became_exact_with_regression_1205) {
                pre_regression_1206_case_id_hash.update(case.id);
                pre_regression_1206_case_id_hash.update("\x00");
            }
            const became_exact_with_regression_1182 = std.mem.eql(
                u8,
                case.id,
                "stylus-official-regression-1182",
            ) or became_exact_with_regression_1205;
            if (!became_exact_with_regression_1182) {
                pre_regression_1205_case_id_hash.update(case.id);
                pre_regression_1205_case_id_hash.update("\x00");
            }
            const became_exact_with_property_access =
                ...
                std.mem.eql(u8, case.id, "stylus-official-property-access-siblings") or
                became_exact_with_regression_1182;
            if (!became_exact_with_property_access) {
                pre_regression_1182_case_id_hash.update(case.id);
                pre_regression_1182_case_id_hash.update("\x00");
            }
            ...
            const became_exact_with_parse = std.mem.eql(
                u8,
                case.id,
                "stylus-official-parse",
            ) or became_exact_with_properties;
            if (!became_exact_with_parse) {
                pre_properties_case_id_hash.update(case.id);
                pre_properties_case_id_hash.update("\x00");
            }
            const became_exact_with_unary = std.mem.eql(
                u8,
                case.id,
                "stylus-official-operators-unary",
            ) or became_exact_with_parse;
            if (!became_exact_with_unary) {
                pre_parse_case_id_hash.update(case.id);
                pre_parse_case_id_hash.update("\x00");
            }
            const became_exact_with_subscript_range = std.mem.eql(
                u8,
                case.id,
                "stylus-official-operators-subscript-range",
            ) or became_exact_with_unary;
            if (!became_exact_with_subscript_range) {
                pre_unary_case_id_hash.update(case.id);
                pre_unary_case_id_hash.update("\x00");
            }
            const became_exact_with_subscript_assignment = std.mem.eql(
                u8,
                case.id,
                "stylus-official-operators-subscript-assign",
            ) or became_exact_with_subscript_range;
            if (!became_exact_with_subscript_assignment) {
                pre_subscript_range_case_id_hash.update(case.id);
                pre_subscript_range_case_id_hash.update("\x00");
            }
            const became_exact_with_subscript = std.mem.eql(
                u8,
                case.id,
                "stylus-official-operators-subscript",
            ) or became_exact_with_subscript_assignment;
            if (!became_exact_with_subscript) {
                pre_subscript_assignment_case_id_hash.update(case.id);
                pre_subscript_assignment_case_id_hash.update("\x00");
            }
            ...
            const became_exact_with_membership = std.mem.eql(
                u8,
                case.id,
                "stylus-official-operators-in",
            ) or became_exact_with_precedence;
            if (!became_exact_with_membership) {
                pre_precedence_case_id_hash.update(case.id);
                pre_precedence_case_id_hash.update("\x00");
            }
            const became_exact_with_equality = std.mem.eql(
                u8,
                case.id,
                "stylus-official-operators-equality",
            ) or became_exact_with_membership;
            if (!became_exact_with_equality) {
                pre_membership_case_id_hash.update(case.id);
                pre_membership_case_id_hash.update("\x00");
            }
            const became_exact_with_complex_operators = std.mem.eql(
                u8,
                case.id,
                "stylus-official-operators-complex",
            ) or became_exact_with_equality;
            if (!became_exact_with_complex_operators) {
                pre_equality_case_id_hash.update(case.id);
                pre_equality_case_id_hash.update("\x00");
            }
            const became_exact_with_root_assignment = std.mem.eql(
                u8,
                case.id,
                "stylus-official-operators-assignment-root",
            ) or became_exact_with_complex_operators;
            if (!became_exact_with_root_assignment) {
                pre_complex_operators_case_id_hash.update(case.id);
                pre_complex_operators_case_id_hash.update("\x00");
            }
            const became_exact_with_operators =
                std.mem.eql(u8, case.id, "stylus-official-operators") or
                std.mem.eql(u8, case.id, "stylus-official-operators-mixins") or
                became_exact_with_root_assignment;
            if (!became_exact_with_operators) {
                pre_operators_case_id_hash.update(case.id);
                pre_operators_case_id_hash.update("\x00");
            }
            const became_exact_with_operator_range =
                std.mem.eql(u8, case.id, "stylus-official-operator-range") or
                became_exact_with_operators;
            if (!became_exact_with_operator_range) {
                pre_operator_range_case_id_hash.update(case.id);
                pre_operator_range_case_id_hash.update("\x00");
            }
            const became_exact_with_object_mixin =
                std.mem.eql(u8, case.id, "stylus-official-object-mixin") or
                became_exact_with_operator_range;
            if (!became_exact_with_object_mixin) {
                pre_object_mixin_case_id_hash.update(case.id);
                pre_object_mixin_case_id_hash.update("\x00");
            }
            const became_exact_with_object_complex =
                std.mem.eql(u8, case.id, "stylus-official-object-complex") or
                became_exact_with_object_mixin;
            if (!became_exact_with_object_complex) {
                pre_object_complex_case_id_hash.update(case.id);
                pre_object_complex_case_id_hash.update("\x00");
            }
            const became_exact_with_multiline =
                std.mem.eql(u8, case.id, "stylus-official-multiline") or
                std.mem.eql(u8, case.id, "stylus-official-regression-839") or
                became_exact_with_object_complex;
            if (!became_exact_with_multiline) {
                pre_multiline_case_id_hash.update(case.id);
                pre_multiline_case_id_hash.update("\x00");
            }
            const became_exact_with_mixins_complex =
                std.mem.eql(u8, case.id, "stylus-official-mixins-complex") or
                became_exact_with_multiline;
            if (!became_exact_with_mixins_complex) {
                pre_mixins_complex_case_id_hash.update(case.id);
                pre_mixins_complex_case_id_hash.update("\x00");
            }
            const became_exact_with_media_complex =
                std.mem.eql(u8, case.id, "stylus-official-media-complex") or
                std.mem.eql(u8, case.id, "stylus-official-mixins-complex") or
                became_exact_with_multiline;
            if (!became_exact_with_media_complex) {
                pre_media_complex_case_id_hash.update(case.id);
                pre_media_complex_case_id_hash.update("\x00");
            }
            const became_exact_with_media_bubbling =
                std.mem.eql(u8, case.id, "stylus-official-media-bubble") or
                std.mem.eql(u8, case.id, "stylus-official-regression-1727") or
                std.mem.eql(u8, case.id, "stylus-official-regression-1882") or
                std.mem.eql(u8, case.id, "stylus-official-media-complex") or
                std.mem.eql(u8, case.id, "stylus-official-mixins-complex") or
                became_exact_with_multiline;
            if (!became_exact_with_media_bubbling) {
                pre_media_bubble_case_id_hash.update(case.id);
                pre_media_bubble_case_id_hash.update("\x00");
            }
        } else {
            nonconforming_count += 1;
            if (first_nonconforming_id == null) first_nonconforming_id = case.id;
    ...
    if (exact_success_count != 326 or nonconforming_count != 0 or
        exact_hash != 0x3b55c78d94378874)
    {
        std.debug.print(
            ...
            .{
                ...
                nonconforming_count,
                exact_hash,
                first_nonconforming_id orelse "<none>",
            },
        );
    }
    ...
    try std.testing.expectEqual(@as(usize, 326), exact_success_count);
    try std.testing.expectEqual(@as(usize, 0), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x3b55c78d94378874), exact_hash);
    try std.testing.expectEqual(
        @as(u64, 0x4c6285fa03b40abb),
    ...
    try std.testing.expectEqual(
        @as(u64, 0x9acbddef87ac2e02),
        pre_variables_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x6fa4f5ba730b584c),
        pre_supports_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xfc86f933324fe58d),
        pre_selectors_nested_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xdbbd6ca7bcb8d343),
        pre_selectors_complex_case_id_hash.final(),
    );
    try std.testing.expectEqual(
    ...
    try std.testing.expectEqual(
        ...
        pre_regression_499_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xa9b4c64f538dff51),
        pre_regression_498_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xaccc633b642562e0),
        pre_regression_480_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xa09332be4223b1f4),
        pre_regression_475_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x3ec52e5d436c3c97),
        pre_regression_432_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xaad76648ea32185d),
        pre_regression_2820_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xc979a303fb1f755e),
        pre_regression_274_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x1670f5be5abd794d),
        pre_regression_272_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x462dc405a159a660),
        pre_regression_216_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x3bd0472031f4f0df),
        pre_regression_1997_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x6139ca6a6f2b853e),
        pre_regression_1584_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xc98601714db350e7),
        pre_regression_1572_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x10c454cff99e1f65),
        pre_regression_156_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x18595fad24d0cd29),
        pre_regression_142_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xb5e0f5e285eb92fd),
        pre_regression_137_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xa567d3986b454016),
        pre_regression_131_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x6e995a9f218f911e),
        pre_regression_130_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x69296b38c640eb03),
        pre_regression_1277_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xa312a42b47bdc152),
        pre_regression_1206_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x564cd920659fde5e),
        pre_regression_1205_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x9a89e6cfeb773571),
        pre_regression_1182_case_id_hash.final(),
    );
    try std.testing.expectEqual(
    ...
    try std.testing.expectEqual(
        ...
        pre_properties_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x7fd5cc165683f885),
        pre_parse_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x2076ad4dd3a15b28),
        pre_unary_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xcc6e7870843659d1),
        pre_subscript_range_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x7c9c36b38140cdee),
        pre_subscript_assignment_case_id_hash.final(),
    );
    try std.testing.expectEqual(
    ...
    try std.testing.expectEqual(
        ...
        pre_precedence_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xb35c69cda93e73ce),
        pre_membership_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xd53dfca8c0cc9c18),
        pre_equality_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x5d31fef38324fcd1),
        pre_complex_operators_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x956486de057f085c),
        pre_operators_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xe311f0398c17d8fe),
        pre_operator_range_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xe4c57f83224c7462),
        pre_object_mixin_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xecd638eb43f8a071),
        pre_object_complex_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xb61c2417bc249721),
        pre_multiline_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0xc0a5c05d6b7f91c5),
        pre_mixins_complex_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x9a4d594e6013c758),
        pre_media_complex_case_id_hash.final(),
    );
    try std.testing.expectEqual(
        @as(u64, 0x9c8106b53217cf87),
        pre_media_bubble_case_id_hash.final(),
    );
    try std.testing.expect(first_nonconforming_id == null);
}
...
test "native Stylus closes the finite literal conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    const case = try findCase(parsed.value.cases, "stylus-official-literal");
    try std.testing.expectEqualStrings("literal", case.feature);
    try std.testing.expectEqualStrings("success", case.outcome);
    try std.testing.expectEqualStrings("expanded", case.style);
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus literal mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite literal color conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    const case = try findCase(parsed.value.cases, "stylus-official-literal-color");
    try std.testing.expectEqualStrings("literal", case.feature);
    try std.testing.expectEqualStrings("success", case.outcome);
    try std.testing.expectEqualStrings("expanded", case.style);
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus literal color mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite media bubble conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    const case = try findCase(parsed.value.cases, "stylus-official-media-bubble");
    try std.testing.expectEqualStrings("media", case.feature);
    try std.testing.expectEqualStrings("success", case.outcome);
    try std.testing.expectEqualStrings("expanded", case.style);
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus media bubble mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite media complex conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    const case = try findCase(parsed.value.cases, "stylus-official-media-complex");
    try std.testing.expectEqualStrings("media", case.feature);
    try std.testing.expectEqualStrings("success", case.outcome);
    try std.testing.expectEqualStrings("expanded", case.style);
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus media complex mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite mixins complex conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    const case = try findCase(parsed.value.cases, "stylus-official-mixins-complex");
    try std.testing.expectEqualStrings("mixins", case.feature);
    try std.testing.expectEqualStrings("success", case.outcome);
    try std.testing.expectEqualStrings("expanded", case.style);
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus mixins complex mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite multiline conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    const case = try findCase(parsed.value.cases, "stylus-official-multiline");
    try std.testing.expectEqualStrings("multiline", case.feature);
    try std.testing.expectEqualStrings("success", case.outcome);
    try std.testing.expectEqualStrings("expanded", case.style);
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus multiline mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite object complex conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    const case = try findCase(parsed.value.cases, "stylus-official-object-complex");
    try std.testing.expectEqualStrings("object", case.feature);
    try std.testing.expectEqualStrings("success", case.outcome);
    try std.testing.expectEqualStrings("expanded", case.style);
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus object complex mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite object mixin conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    const case = try findCase(parsed.value.cases, "stylus-official-object-mixin");
    try std.testing.expectEqualStrings("object", case.feature);
    try std.testing.expectEqualStrings("success", case.outcome);
    try std.testing.expectEqualStrings("expanded", case.style);
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus object mixin mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite operator range conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    const case = try findCase(parsed.value.cases, "stylus-official-operator-range");
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus operator range mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite operators conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    const case = try findCase(parsed.value.cases, "stylus-official-operators");
    try std.testing.expectEqualStrings("operators", case.feature);
    try std.testing.expectEqualStrings("success", case.outcome);
    try std.testing.expectEqualStrings("expanded", case.style);
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus operators mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite root assignment conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus root assignment mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite complex operators conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus complex operators mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite equality conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus equality mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite membership conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus membership mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite operator precedence conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
...
test "native Stylus closes the finite operator subscript assignment conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus operator subscript assignment mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite operator subscript range conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus operator subscript range mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite operator unary conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus operator unary mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite parse conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    try std.testing.expectEqualStrings("parse", case.feature);
    try std.testing.expectEqualStrings("success", case.outcome);
    try std.testing.expectEqualStrings("expanded", case.style);
...
test "native Stylus closes the finite properties conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    try std.testing.expectEqualStrings("properties", case.feature);
    try std.testing.expectEqualStrings("success", case.outcome);
    try std.testing.expectEqualStrings("expanded", case.style);
...
test "native Stylus closes the finite regression 1182 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 1182 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 1205 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 1205 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 1206 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 1206 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 1277 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    const case = try findCase(
        parsed.value.cases,
        "stylus-official-regression-1277",
    );
    try std.testing.expectEqualStrings("regression", case.feature);
    try std.testing.expectEqualStrings("success", case.outcome);
    try std.testing.expectEqualStrings("expanded", case.style);
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 1277 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 130 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    const case = try findCase(
        parsed.value.cases,
        "stylus-official-regression-130",
    );
    try std.testing.expectEqualStrings("regression", case.feature);
    try std.testing.expectEqualStrings("success", case.outcome);
    try std.testing.expectEqualStrings("expanded", case.style);
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 130 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 131 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 131 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 137 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 137 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 142 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 142 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 156 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 156 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 1572 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 1572 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 1584 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 1584 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 1997 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 1997 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 216 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 216 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 272 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 272 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 274 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 274 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 2820 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 2820 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 432 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 432 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 475 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 475 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 480 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 480 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 498 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus regression 498 mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite regression 499 conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
...
test "native Stylus closes the finite complex selectors conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    try std.testing.expectEqualStrings("selectors", case.feature);
    try std.testing.expectEqualStrings("success", case.outcome);
    try std.testing.expectEqualStrings("expanded", case.style);
    ...
    defer expected_css.deinit();
    var terminal = stylus_evaluator.Limits{};
    terminal.max_source_bytes = input.len;
    terminal.max_selectors = 20;
    var first = try compileNativeWithLimits(allocator, case, input, terminal);
    defer first.deinit();
    var second = try compileNativeWithLimits(allocator, case, input, terminal);
    defer second.deinit();
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus complex selectors mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite nested selectors conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus nested selectors mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite supports conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    try std.testing.expectEqualStrings("supports", case.feature);
    try std.testing.expectEqualStrings("success", case.outcome);
    try std.testing.expectEqualStrings("expanded", case.style);
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus supports mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
...
test "native Stylus closes the finite variables conformance family" {
    const allocator = std.testing.allocator;
    const manifest_bytes = try std.fs.cwd().readFileAlloc(
        allocator,
        "tests/preprocessors/stylus/corpus/manifest.json",
    ...
    std.testing.expectEqualStrings(expected_css.css(), first.css()) catch |failure| {
        std.debug.print(
            "\nnative Stylus variables mismatch\nexpected: {s}\nactual:   {s}\n",
            .{ expected_css.css(), first.css() },
        );
        return failure;
    };
