...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    try std.testing.expectEqual(@as(usize, 201), exact_success_count);
    try std.testing.expectEqual(@as(usize, 125), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x91cee7029a7ada4e), exact_case_id_hash.final());
}
