...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_loop_extension = std.mem.eql(
                u8,
                case.id,
                "stylus-official-extend-in-loop",
            );
            if (!became_exact_with_loop_extension) {
    ...
    if (exact_success_count != 227 or nonconforming_count != 99 or
        exact_hash != 0x1effbdee66b76523)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 227), exact_success_count);
    try std.testing.expectEqual(@as(usize, 99), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x1effbdee66b76523), exact_hash);
    try std.testing.expectEqual(
        @as(u64, 0xfe3d6ca881fc5226),
        previous_exact_case_id_hash.final(),
    );
    try std.testing.expectEqualStrings(
        "stylus-official-extend-in-loop-context",
