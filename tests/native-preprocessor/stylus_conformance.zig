...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    if (exact_success_count != 305 or nonconforming_count != 21 or
        exact_hash != 0x18595fad24d0cd29)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 305), exact_success_count);
    try std.testing.expectEqual(@as(usize, 21), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x18595fad24d0cd29), exact_hash);
