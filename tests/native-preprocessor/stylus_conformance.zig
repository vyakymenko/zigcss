...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    try std.testing.expectEqual(@as(usize, 190), exact_success_count);
    try std.testing.expectEqual(@as(usize, 136), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x8b463bf4292257ed), exact_case_id_hash.final());
}
