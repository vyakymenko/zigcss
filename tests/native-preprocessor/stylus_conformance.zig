...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_operators =
                ...
                std.mem.eql(u8, case.id, "stylus-official-operators-mixins");
    ...
    if (exact_success_count != 283 or nonconforming_count != 43 or
        exact_hash != 0x5d31fef38324fcd1)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 283), exact_success_count);
    try std.testing.expectEqual(@as(usize, 43), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x5d31fef38324fcd1), exact_hash);
