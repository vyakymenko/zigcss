...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_loop_context_extension = std.mem.eql(
                u8,
                case.id,
                "stylus-official-extend-in-loop-context",
            );
            if (!became_exact_with_loop_context_extension) {
    ...
    if (exact_success_count != 228 or nonconforming_count != 98 or
        exact_hash != 0xf69503904705a517)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 228), exact_success_count);
    try std.testing.expectEqual(@as(usize, 98), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0xf69503904705a517), exact_hash);
    try std.testing.expectEqual(
        @as(u64, 0x1effbdee66b76523),
