...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_css_selector_interpolation = std.mem.eql(
                u8,
                case.id,
                "stylus-official-css-selector-interpolation",
            );
            if (!became_exact_with_css_selector_interpolation) {
    ...
    if (exact_success_count != 221 or nonconforming_count != 105 or
        exact_hash != 0x6860b8d1939167e4)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 221), exact_success_count);
    try std.testing.expectEqual(@as(usize, 105), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x6860b8d1939167e4), exact_hash);
    try std.testing.expectEqual(
        @as(u64, 0x1adb685ce18576a4),
