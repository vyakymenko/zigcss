...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    try std.testing.expectEqual(@as(usize, 191), exact_success_count);
    try std.testing.expectEqual(@as(usize, 135), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x5582119ab3760aca), exact_case_id_hash.final());
}
