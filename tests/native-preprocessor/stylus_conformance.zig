...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_css_mixins_root_wonky = std.mem.eql(
                u8,
                case.id,
                "stylus-official-css-mixins-root-wonky",
            );
            if (!became_exact_with_css_mixins_root_wonky) {
    ...
    if (exact_success_count != 220 or nonconforming_count != 106 or
        exact_hash != 0x1adb685ce18576a4)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 220), exact_success_count);
    try std.testing.expectEqual(@as(usize, 106), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x1adb685ce18576a4), exact_hash);
    try std.testing.expectEqual(
        @as(u64, 0x5e87a9d2dfa920cd),
