...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_regression_130 =
                ...
                std.mem.eql(u8, case.id, "stylus-official-regression-1998");
    ...
    if (exact_success_count != 304 or nonconforming_count != 22 or
        exact_hash != 0xb5e0f5e285eb92fd)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 304), exact_success_count);
    try std.testing.expectEqual(@as(usize, 22), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0xb5e0f5e285eb92fd), exact_hash);
