...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_indentation_at_rules = std.mem.eql(
                u8,
                case.id,
                "stylus-official-fontface",
            );
            if (!became_exact_with_indentation_at_rules) {
    ...
    if (exact_success_count != 237 or nonconforming_count != 89 or
        exact_hash != 0x315d3a5f03dad7b4)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 237), exact_success_count);
    try std.testing.expectEqual(@as(usize, 89), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x315d3a5f03dad7b4), exact_hash);
    try std.testing.expectEqual(
        @as(u64, 0xf69f62f1a1950ff8),
        previous_exact_case_id_hash.final(),
    );
    try std.testing.expectEqualStrings(
        "stylus-official-for-complex",
