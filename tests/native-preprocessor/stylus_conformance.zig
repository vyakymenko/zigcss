...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_object_complex =
                std.mem.eql(u8, case.id, "stylus-official-object-complex");
    ...
    if (exact_success_count != 279 or nonconforming_count != 47 or
        exact_hash != 0xe4c57f83224c7462)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 279), exact_success_count);
    try std.testing.expectEqual(@as(usize, 47), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0xe4c57f83224c7462), exact_hash);
