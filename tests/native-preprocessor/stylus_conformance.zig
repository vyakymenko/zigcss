...
test "native Stylus measures the finite pinned success corpus without ordinal expansion" {
    ...
    for (parsed.value.cases) |case| {
        ...
        if (equivalent and std.mem.eql(u8, expected_css.css(), first.css())) {
            ...
            const became_exact_with_variable_extension_target = std.mem.eql(
                u8,
                case.id,
                "stylus-official-extend-using-variable",
            );
            if (!became_exact_with_variable_extension_target) {
    ...
    if (exact_success_count != 234 or nonconforming_count != 92 or
        exact_hash != 0x2e705eb22f31713b)
    {
        std.debug.print(
    ...
    try std.testing.expectEqual(@as(usize, 234), exact_success_count);
    try std.testing.expectEqual(@as(usize, 92), nonconforming_count);
    try std.testing.expectEqual(@as(u64, 0x2e705eb22f31713b), exact_hash);
    try std.testing.expectEqual(
        @as(u64, 0x62cf4c87c93727f3),
