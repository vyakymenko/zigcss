...
| Alternate-format silent deletion and guessed parsing | Contained by `FMT-001`; eight adapters/nine extensions are machine-inventoried and real CLI probes exit 2 without output. `SCSS-001` removes both Sass parser sources/dispatch tags and makes the test-only detector return null; remaining adapters retain named owners. | `FMT-001`, `SCSS-001`, and the named follow-on adapter packages |
