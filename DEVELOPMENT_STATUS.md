...
- Work package: Published grammar matrix and independent parser validation
- State: `IN_PROGRESS`
- Next eligible package: Published grammar matrix and independent parser validation
...
Milestones 0 and 1 are `PASS`. Latest package validation (`M2-CLI`): Debug and ReleaseSafe each pass 268/268 tests (80 legacy unit, 157 new library/core, 31 CLI integration); Debug and ReleaseSafe builds complete; `src/css.zig`, `src/css/pipeline.zig`, and `tests/regressions/audit.zig` pass focused formatting, and the semantic diff passes whitespace checks. The stable CLI now uses the new parser/emitter and no audit parser/minifier quarantine remains. The same 19 inherited formatting failures and recorded dependency-audit debt remain scheduled work. The Milestone 2 exit gate remains active for its published grammar matrix and independent validation requirements.
