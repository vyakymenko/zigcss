...
pub fn build(b: *std.Build) void {
    ...
    const native_compiler_test_module = b.createModule(.{
        .root_source_file = b.path("tests/native-preprocessor/compiler.zig"),
        .target = target,
        .optimize = optimize,
    });
    native_compiler_test_module.addImport("native_preprocessor", native_preprocessor_module);
    const native_compiler_tests = b.addTest(.{
        .root_module = native_compiler_test_module,
    });
    const run_native_compiler_tests = b.addRunArtifact(native_compiler_tests);
    run_native_compiler_tests.setCwd(b.path("."));
    const native_compiler_test_step = b.step(
        "test-native-compiler",
        "Test the private shared native stylesheet compiler route",
    );
    native_compiler_test_step.dependOn(&run_native_compiler_tests.step);
    ...
    const native_stylus_parser_test_module = b.createModule(.{
        .root_source_file = b.path("tests/native-preprocessor/stylus_parser.zig"),
        .target = target,
        .optimize = optimize,
    });
    native_stylus_parser_test_module.addImport("native_preprocessor", native_preprocessor_module);
    const native_stylus_parser_tests = b.addTest(.{
        .root_module = native_stylus_parser_test_module,
    });
    const run_native_stylus_parser_tests = b.addRunArtifact(native_stylus_parser_tests);
    run_native_stylus_parser_tests.setCwd(b.path("."));
    const native_stylus_parser_test_step = b.step(
        "test-native-stylus-parser",
        "Test the private native Stylus syntax parser",
    );
    native_stylus_parser_test_step.dependOn(&run_native_stylus_parser_tests.step);
    const native_stylus_evaluator_test_module = b.createModule(.{
        .root_source_file = b.path("tests/native-preprocessor/stylus_evaluator.zig"),
        .target = target,
        .optimize = optimize,
    });
    native_stylus_evaluator_test_module.addImport("native_preprocessor", native_preprocessor_module);
    const native_stylus_evaluator_tests = b.addTest(.{
        .root_module = native_stylus_evaluator_test_module,
    });
    const run_native_stylus_evaluator_tests = b.addRunArtifact(native_stylus_evaluator_tests);
    run_native_stylus_evaluator_tests.setCwd(b.path("."));
    const native_stylus_evaluator_test_step = b.step(
        "test-native-stylus-evaluator",
    ...
    native_less_evaluator_test_module.addImport("native_preprocessor", native_preprocessor_module);
    const native_less_evaluator_tests = b.addTest(.{
        .root_module = native_less_evaluator_test_module,
    });
    const run_native_less_evaluator_tests = b.addRunArtifact(native_less_evaluator_tests);
    const native_less_evaluator_test_step = b.step(
        "test-native-less-evaluator",
        "Test the private native Less semantic evaluator",
    );
    native_less_evaluator_test_step.dependOn(&run_native_less_evaluator_tests.step);
    const native_less_conformance_test_module = b.createModule(.{
        .root_source_file = b.path("tests/native-preprocessor/less_conformance.zig"),
        .target = target,
        .optimize = optimize,
    });
    native_less_conformance_test_module.addImport("native_preprocessor", native_preprocessor_module);
    const native_less_conformance_tests = b.addTest(.{
        .root_module = native_less_conformance_test_module,
    });
    const run_native_less_conformance_tests = b.addRunArtifact(native_less_conformance_tests);
    run_native_less_conformance_tests.setCwd(b.path("."));
    const native_less_conformance_test_step = b.step(
        "test-native-less-conformance",
        "Test the pinned native Less conformance evidence",
    );
    native_less_conformance_test_step.dependOn(&run_native_less_conformance_tests.step);
    ...
    const native_sass_conformance_test_module = b.createModule(.{
        .root_source_file = b.path("tests/native-preprocessor/sass_conformance.zig"),
        .target = target,
        .optimize = optimize,
    });
    native_sass_conformance_test_module.addImport("native_preprocessor", native_preprocessor_module);
    const native_sass_conformance_tests = b.addTest(.{
        .root_module = native_sass_conformance_test_module,
    });
    const run_native_sass_conformance_tests = b.addRunArtifact(native_sass_conformance_tests);
    run_native_sass_conformance_tests.setCwd(b.path("."));
    const native_sass_conformance_test_step = b.step(
        "test-native-sass-conformance",
        "Test the pinned native Sass conformance evidence",
    );
    native_sass_conformance_test_step.dependOn(&run_native_sass_conformance_tests.step);
    ...
    native_preprocessor_test_step.dependOn(&run_native_compiler_tests.step);
    ...
    native_preprocessor_test_step.dependOn(&run_native_sass_conformance_tests.step);
    ...
    test_step.dependOn(&run_native_compiler_tests.step);
