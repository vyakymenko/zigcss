...
| CSS Modules | Experimental, Zig-library-only native subset | Explicit `.syntax = .css_modules` produces file-specific deterministic class names and owned exports through the rebuilt pipeline. The CLI/LSP still reject `.module.css`; deferred module semantics are strict errors. |
...
The stable CSS path returns `module_exports = null`. Explicit `.syntax = .css_modules` selects the [experimental native subset](docs/src/content/docs/guide/css-modules.md): local-by-default typed class selectors receive versioned source-specific SHA-256 names and the result owns unique first-seen name/value exports. The CLI remains CSS-only. Fixed-point optimization and source maps cannot be combined until intermediate maps can be composed; CSS Modules currently reject optimizer/prefix/plugin composition; incoherent options return owned `API0001` diagnostics without CSS.
...
Property-specific values are usually preserved as lossless component trees rather than fully validated semantics. Browser computed-style validation, broader prefix data and prefix CLI exposure, element/attribute or dynamic-DOM extraction, optimized source-map composition, CLI source-map output, LSP migration, broader CSS Modules semantics, other alternate formats, and resource-bounded public compilation remain incomplete.
