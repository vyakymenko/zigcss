...
| LSP | Experimental | Bounded dynamic framing accepts sequential CRLF `Content-Length` messages with 8 KiB headers and bodies up to 16 MiB. Standard JSON parsing/serialization escapes IDs and handler data and returns JSON-RPC parse, request, method, and parameter errors; notification semantics, lifecycle, positions, features, and the legacy parser remain later gates. |
