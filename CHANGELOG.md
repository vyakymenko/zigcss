...
The provider-backed `0.5.0-rc.1` historical reference identity remains ineligible for publication. The native `0.6.0-rc.1` candidate failed closed during automatic Windows hosted validation before any tag or publication, so it remains immutable failure evidence. Replacement candidate `0.6.0-rc.2` was selected only after its GitHub tag and npm version were proven unused; its release interlock remains closed pending the remaining `NATIVE-009` evidence.
